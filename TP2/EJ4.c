// Para implementar correctamente la aritmética de saturación, es un requisito indispensable operar primero con un acumulador más largo (en este caso, de 64 bits) antes de aplicar los límites
// . Esto permite que la suma retenga su valor real temporalmente para luego poder compararlo contra INT_MAX e INT_MIN y decidir si es necesario saturar el resultado, previniendo así la catastrófica "caída en escalera" o rollover
// .
// Código en C
#include <stdio.h>
#include <stdint.h>
#include <limits.h> // Contiene las macros INT_MAX e INT_MIN

// 1. Función de suma con aritmética de saturación
int32_t saturation(int32_t a, int32_t b) {
    // Se utiliza un acumulador de 64 bits para evitar el overflow intermedio [1]
    int64_t acumulador = (int64_t)a + (int64_t)b;

    // Se satura el valor si supera los límites permitidos para 32 bits [1, 3]
...
int main(void) {
    ...
    b = 500;
    res_saturado = saturation(a, b);
    res_normal = a + b; // Suma directa para mostrar el rollover
...
// Caso 1 (Normal): Suma dos números cuyos valores están dentro de los márgenes soportados. El acumulador simplemente se convierte de nuevo a 32 bits, comportándose de manera completamente lineal
// .
